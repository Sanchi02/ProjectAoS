        ...
        for i in range(0,26,2):
            for val in dicty:
                for node in dicty[val]:
                    ...
                    weight = '{0:.3f}'.format(0.5*(bandwidth/packet))
                    allRows.append([tmpID, val,node,bandwidth, packet, day, i, weight])
                    tmpID += 1
    df = pd.DataFrame(allRows,columns=['ID','EP1','EP2', 'Bandwidth', 'PacketsFlowing(1Kb)','DayOfTheWeek', 'TimeHour', 'LinkWeight'])
    stringy = "snapshotForWeek"+str(_)+".csv"
    df.to_csv(stringy, index=True)

        

# df = pd.DataFrame(allRows,columns=['EP1','EP2', 'Bandwidth', 'PacketsFlowing(1Kb)','DayOfTheWeek', 'TimeHour'])
